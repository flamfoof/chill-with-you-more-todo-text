...
I made this because the to-do list only lets you type a tiny bit of text per task, and once you go past a certain point it just chops everything off with a "..." at the end. I wanted to actually write out longer tasks and notes without it cutting me off.
...
It uses BepInEx. Just download the latest release and unzip it anywhere. Run the .bat or .ps1 file by double clicking it, point it at the game's .exe folder, and hit Install/Update Mod.
...
[*][b]DisableInputScroll[/b] = stops a box from scrolling its own contents / grabbing the mouse wheel so the list scrolls normally. Also fixes pasted text shifting out of position (default true).
...
It's all done in memory at runtime, so it doesn't touch or replace any of the game's files. It's easy to remove (just delete the plugin) and should keep working across small game updates. If a bigger update ever moves things around I'll patch it. This also plays nice alongside my "more music" mod since they don't touch the same parts of the game, so you can run both.
